...
def load_data():
    column_names = ['user_id', 'play_id', 'rating']
    ratings = pd.read_csv('mock_mlb_data.csv', names=column_names, skiprows=1)
    return ratings
...
def main():
    ratings = load_data()
    user_play_matrix = ratings.pivot_table(index='user_id', columns='play_id', values='rating', fill_value=0)
